...
import {
  ...
  AnchorNode,
  ImageNode,
} from '#lib/models/RichTextDocument';
...
function fromInlineStyle(s: InlineStyle): DraftInlineStyleType {
  switch (s) {
...
function toBlockStyle(s: string): BlockStyle {
  switch (s) {
    ...
    case 'atomic':
      return BlockStyle.Atomic;
    default:
      return BlockStyle.Unstyled;
  }
}
...
function nodeToBlock(node: BlockNode, entityKeyIdx = 0) {
  ...
  for (const nodeChild of node.c) {
    ...
    if (nodeChild.t === NodeType.Anchor) {
      const type = 'LINK';
    ...
    // Same with image nodes
    else if (nodeChild.t === NodeType.Image) {
      const type = 'IMAGE';
      const mutability = 'IMMUTABLE' as const;
      const data = { src: nodeChild.u };
      const entity = { type, mutability, data };
